...
type Props = {
  ...
  tv?: MoviesResults[]
  promisenya: Promise<Movies>
}

export default function SampleList({ movies, tv, promisenya }: Props) {
  const data = use(promisenya)
  movies = data.results

  // const getDataTv: Movies = use(getMoviesPopular())
  // movies = getDataTv.results
...
function CardMovieItem({ data }: PropCardMovieItem) {
  ...
  // const { results: data } = use(promisenya)
